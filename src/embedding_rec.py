"""
Content-based blog recommender using embeddings and OpenAI for ad generation.
"""
...
def recs_to_dataframe(recs, unique_blogs):
    ...
    unique_blogs : pd.DataFrame
        The DataFrame  created earlier with one row per blog and
        cols like ['blog_id', 'blog_title', 'blog_content', etc

    Returns:
    --------
    pd.DataFrame
        Columns: ['blog_id', 'score', 'blog_title', 'blog_content', etc
    """
...
def get_recommendations_for_user(user_id, df, embedding_model, top_n=10):
    ...
    liked_indices = [
        blog_id_to_idx[bid] for bid in liked_blogs if bid in blog_id_to_idx
    ]
...
def generate_personalized_ads(rec_df, user_id, df, unique_blogs, client):
    """
    Generate personalized advertising content for recommendations
    using OpenAI API.
    ...
    if len(liked_blogs) > 0:
        try:
            ...
            liked_with_titles = liked_blogs.merge(
                unique_blogs, on="blog_id", how="left"
            )

            # Check if blog_title column exists
            if "blog_title" in liked_with_titles.columns:
                liked_titles = (
                    liked_with_titles["blog_title"]
                    .dropna()
                    .tolist()
                )
                user_context = (
                    "User previously liked blogs: "
                ...
                top_ids = ', '.join(map(str, liked_ids[:3]))
                user_context = f"User previously liked blog IDs: {top_ids}"
    ...
    Create engaging, personalized blog recommendations
    that feel natural and compelling.
    Your goal is to create a short, catchy advertisement (2-3 sentences)
    that connects the user's interests to the recommended blog.
    Use phrases like "Because you enjoyed..." or "Since you liked..."
    ...
    for idx, row in rec_df.iterrows():
        ...
        blog_content = (
            row["blog_content"][:500]
            if pd.notna(row["blog_content"])
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Create a personalized recommendation ad "
                           f"for this blog: {blog_info}",
            },
        ]
...
# df= pd.read_pickle(os.path.join(processeddata_path,
# 'cleaned_blog_ratings.pkl'))
# embedding_model = load_embedding_model(os.path.join(base_path,"models",
# "embedding_model.pkl"))
# unique_blogs = prepare_blog_data(df)
# recs = get_recommendations_for_user(user_id=123, df=df,
...
# personalized_ads = generate_personalized_ads(rec_df, user_id=123,
# df=df, unique_blogs=unique_blogs, client=client)
...
#          'recommendation_score', 'personalized_ad']
#     ].head()
# )
